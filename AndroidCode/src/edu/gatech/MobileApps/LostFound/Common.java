package edu.gatech.MobileApps.LostFound;

public class Common {
	//preferences keys
	public static final String REMEMBER_KEY = "remember_me";
	public static final String REMEMBER_USER_KEY = "username";
	public static final String REMEMBER_PASS_KEY = "password";
	
	public static final boolean DEBUG = true;
	
	//unchanging values needed in multiple classes
	public static final String MAIN_PREFERENCES = "PREFERENCES";
	public static final String API_DOMAIN = "";
}
